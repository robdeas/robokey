...
object Utils : Logable {
    ...
    const val ENABLE_AUTO_PUBLISH = true // can disable automatic event publishing
    ...
    /**
     * Extracts and matches a command from a given string to an `EventCommand` enum value.
     *
     * This function normalises the input string by:
     * - Limiting the string to a maximum of 32 characters.
     * - Converting all characters to uppercase.
     * - Replacing delimiters (`_`, `-`, `.`, `,`, whitespace) with colons (`:`).
     *
     * After normalisation, it attempts to match the entire string to an `EventCommand` enum value.
     * If no match is found, it extracts the portion before the last colon and attempts to match that instead.
     * If both attempts fail, it defaults to `EventCommand.INVALID`.
     *
     * Edge cases handled:
     * - Empty or blank strings immediately return `EventCommand.INVALID`.
     * - Strings without colons are matched as-is after normalisation.
     *
     * @param command The input string representing a command.
     * @return The corresponding `EventCommand` enum value, or `EventCommand.INVALID` if no match is found.
     */
    fun extractCommandFromString(command: String?): EventCommand {
        // Early return for empty or blank strings
        if (command == null) return EventCommand.UNDEFINED2
        if (command.isBlank()) return EventCommand.UNDEFINED3

        // Normalize the command string
        val normalizedCommand =
            command
                .take(32) // Limit the length to 32 characters
                .uppercase() // Convert to uppercase
                .replace(Regex("[_\\-.,\\s]"), ":") // Replace delimiters with colons

        // Attempt full match
        EventCommand.entries.find { it.name == normalizedCommand }?.let { return it }

        // Attempt to match only the portion before the last colon
        val commandPart = normalizedCommand.substringBeforeLast(":", missingDelimiterValue = normalizedCommand)
        return EventCommand.entries.find { it.name == commandPart } ?: EventCommand.INVALID
    }
}
